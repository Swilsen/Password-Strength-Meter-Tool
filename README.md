...
# Overview
Password strength is based on three things, heuristic factors, binary entropy, and reductions in password score based on predictability and the use of patterns. The entropy score is grounded in hard maths, but the reduction in score due to a limited character set or the inclusion of common password patterns is more subjective. I chose to give harsh reductions in order to better indicate which factors negatively affects password strength. This way the user feedback focuses on promoting a good password hygiene by dynamically and clearly responding to the user's password suggestions.
